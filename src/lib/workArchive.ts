...
async function rowsForRange(range: WorkRange): Promise<WorkRow[]> {
  ...
  for (const raw of operational) {
    ...
    const points = await db.trackPoints.where('sessionId').equals(raw.id).sortBy('timestamp') as TrackPoint[];
...
export async function exportWorkCsv(range: WorkRange): Promise<void> {
  ...
  const headers: Array<keyof WorkRow> = [
    ...
    'gpsPoints', 'product', 'equipment', 'startWeather', 'endWeather'
  ];
  const csv = [
    headers.join(','),
    ...rows.map((row) => headers.map((header) => csvCell(header === 'distanceMiles' ? row[header].toFixed(2) : row[header])).join(','))
  ].join('\n');
...
export async function exportWorkPdf(range: WorkRange): Promise<void> {
  ...
  for (const row of rows) {
    ...
    lines.push(...wrapLine(`  ${row.startedAt}-${row.endedAt} | ${row.durationMinutes} min | ${row.distanceMiles.toFixed(2)} mi | ${row.gpsPoints} points${row.product ? ` | ${row.product}` : ''}${row.equipment ? ` | ${row.equipment}` : ''}`));
